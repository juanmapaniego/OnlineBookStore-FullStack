...
export class BookService {
  ...
  private getBookList(searchUrl: string): Observable<GetResponseBooks> {
  ...
  searchBooks(keyword: string, currentPage: number, pageSize: number): Observable<GetResponseBooks>{
    const searchUrl = `${this.baseUrl}/search/bookname?name=${keyword}&page=${currentPage}&size=${pageSize}`;
